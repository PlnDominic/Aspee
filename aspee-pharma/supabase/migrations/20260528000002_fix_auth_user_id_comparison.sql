-- Fix: all transactional impl functions compared auth_user_id (text) to
-- auth_user_uuid (uuid) using =, which PostgreSQL rejects.
-- Altering the column type is blocked by RLS policies, so we fix it
-- at the call site: cast auth_user_uuid to text in the WHERE/ORDER BY clauses.
-- Every function below is identical to its original except for ::text on those two lines.
...
BEGIN
    ...
    WHERE auth_user_id = auth_user_uuid::text
       OR lower(email) = lower(coalesce(auth_email, ''))
    ORDER BY CASE WHEN auth_user_id = auth_user_uuid::text THEN 0 ELSE 1 END
    LIMIT 1;
