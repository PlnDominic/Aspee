...
export default function QAIncomingMaterialsPage() {
    ...
    const { data: grns, isLoading, refetch } = useSupabaseQuery<any>(
        'grn',
        {
            columns: '*, purchase_orders:po_id(po_number, supplier_id, approved_by, approval_level, suppliers:supplier_id(name)), items:grn_items(*, product:products(name, sku, unit), purchase_order_items:po_item_id(unit_price))',
            orderBy: 'created_at',
            ascending: false
        }
    );
