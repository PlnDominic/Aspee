...
import { Plus, Eye, Trash2, Briefcase, Package, Calendar, Download } from 'lucide-react';
...
const PURPOSE_COLORS: Record<string, 'default' | 'success' | 'warning' | 'danger' | 'info'> = {
    'Office Use': 'default',
    'Testing / Sampling': 'info',
    'Staff Consumption': 'default',
    'Company Promotion': 'success',
    'Research & Development': 'info',
    'Other': 'default',
};
...
export default function InternalUsePage() {
    ...
    const deductStock = async (productId: string, locationId: string, quantity: number, referenceId: string, refNumber: string) => {
        ...
        const { error: movementError } = await supabase
            ...
            .insert([{
                ...
                movement_type: 'OUT',
                quantity,
                reference_type: 'Internal Use',
                reference_id: referenceId,
                notes: `Internal Use ${refNumber}`,
            }]);
    ...
    const saveRecord = useAction<any, void>({
        ...
        invalidateKeys: ['stock_internal_use', 'stock_levels', 'stock-levels-matrix', 'stock_movements'],
        successMessage: 'Internal use logged and stock adjusted',
    });

    const handleDelete = async (id: string) => {
        if (!confirm('Delete this record? Stock will NOT be restored automatically.')) return;
        const { error } = await supabase.from('stock_internal_use').delete().eq('id', id);
        if (error) { toast.error(error.message); return; }
        toast.success('Record deleted');
        queryClient.invalidateQueries({ queryKey: ['stock_internal_use'] });
    };

    const handleExport = () => {
        if (records.length === 0) return;
        const headers = ['Reference', 'Date', 'Product', 'SKU', 'Location', 'Qty', 'Unit', 'Purpose', 'Notes'];
        const csv = [
            headers.join(','),
            ...records.map(r => [
                ...
                `"${r.purpose}"`, `"${(r.notes || '').replace(/"/g, '""')}"`,
            ].join(','))
        ].join('\n');
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `internal_use_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    const totalUnitsUsed = records.reduce((sum, r) => sum + Number(r.quantity || 0), 0);
    ...
    const columns = [
        {
            ...
            label: 'Reference',
            render: (v: any) => <span style={{ fontWeight: 600, color: 'var(--primary-600)', fontFamily: 'var(--font-mono)', fontSize: 12 }}>{v}</span>,
        },
        {
            key: 'date',
            label: 'Date',
        ...
        },
        {
            key: 'purpose',
            label: 'Purpose',
            render: (v: any) => <StatusBadge status={v} variant={PURPOSE_COLORS[v] || 'default'} />,
        },
        {
            key: 'notes',
            label: 'Notes',
            render: (v: any) => <span style={{ fontSize: 12, color: 'var(--slate-500)' }}>{v || '-'}</span>,
        },
        {
            key: 'actions',
            label: 'Actions',
    ...
    return (
        <div className="animate-fade-in">
            <PageHeader
                title="Internal Use"
                subtitle="Track stock consumed for internal company purposes — office use, sampling, staff consumption"
                breadcrumbs={[
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
                <StatCard title="Total Records" value={records.length.toString()} icon={<Briefcase size={20} />} color="blue" />
                <StatCard title="Total Units Used" value={totalUnitsUsed.toLocaleString()} icon={<Package size={20} />} color="amber" />
                <StatCard title="Unique Products" value={uniqueProducts.toString()} icon={<Calendar size={20} />} color="teal" />
            </div>
            ...
            <DataTable
                ...
                data={records}
                loading={loading}
                searchPlaceholder="Search by product, reference, or purpose..."
                emptyMessage="No internal use records found. Click 'Log Internal Use' to add one."
            />
